...
<head>
	...
	<title>学生列表</title>
	<%--可以用${APP_PATH}取代http://localhost:3306/student_crud}--%>
	<%--绝对路径--request.getContextPath()可以返回当前页面所在的根目录的名字--%>
	<%
...
<!-- 搭建显示页面-使用的是bootstrap样式 -->
	<div class="container">
		<!-- 标题 -->
		<div class="row">
			<div class="col-md-4 col-md-offset-4"><%--只占四列且偏移了四列--%>
				<h1>软件六班学生信息表</h1>
			</div>
		</div>
		<br/>
		<!-- 按钮 -->
		<div class="row">
			<div class="col-md-4"><%--这一行只占四列--%>
				<button class="btn btn-primary" id="emp_add_modal_btn">新增</button><%--蓝色按钮--%>
				<button class="btn btn-danger" id="emp_delete_all_btn">删除</button><%--红色按钮--%>
			</div>
		</div>
		<br/>
		<!-- 显示表格数据 -->
...
<!-- 定义页面元素的功能 -->
	...
	<!-- 查询、分页功能 -->
		...
		function build_emps_table(result){
			...
			$.each(emps, function(index,item){
				...
				$("<tr></tr>").append(checkBoxTd)
					...
					.append(deptNameTd)
					.append(btnTd)
					.appendTo("#emps_table tbody");/*添加到表格体中的tbody标签内*/
			});
		}
		...
		function build_page_nav(result){
			...
			//前一页
			var prePageLi = $("<li></li>").append($("<a></a>").append("&laquo;"));/*&laquo这个表示左尖括号*/
			...
			//后一页
			var nextPageLi = $("<li></li>").append($("<a></a>").append("&raquo;"));/*&raquo这个表示右尖括号*/
		...
		//清空模态框样式及内容(Bootstrap代码)-拿到DOM对象，调用reset方法，清空内容
		...
		//使用正则表达式校验添加学生模态框的邮箱是否符合指定格式-前端 jQuery 校验表单数据是否合法
		...
		$("#emp_update_btn").click(function(){
			...
			$.ajax({
                ...
                //需要在web.xml上配置HttpPutFormContentFilter，他的作用是将请求体中的数据解析包装成一个map
                //request方法被重新包装，request.getParameter()被重写就会从自己封装的map中取数据。
				type:"PUT",
		...
		$("#check_all").click(function(){
            /*dom原生的属性可以用prop()修改和读取，返回true或false*/
